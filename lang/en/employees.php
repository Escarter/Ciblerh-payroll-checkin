<?php

return [
    /*
    |--------------------------------------------------------------------------
    | Employees Management Language Lines
    |--------------------------------------------------------------------------
    */

    'employees' => 'Employees',
    'employees_short' => 'Staff',
    'employees_management' => 'Employees Management',
    'employee' => 'Employee',
    'create_manager' => 'Create Manager',
    'total_employees' => 'Employees',
    'create_employee' => 'Create Employee',
    'edit_employee' => 'Edit Employee',
    'employee_created_successfully' => 'Employee created successfully!',
    'employee_updated_successfully' => 'Employee successfully updated!',
    'employee_deleted_successfully' => 'Employee successfully deleted!',
    'employee_restored_successfully' => 'Employee successfully restored!',
    'employee_permanently_deleted' => 'Employee permanently deleted!',
    'selected_employees_moved_to_trash' => 'Selected employees moved to trash!',
    'selected_employees_restored' => 'Selected employees restored!',
    'selected_employees_permanently_deleted' => 'Selected employees permanently deleted!',
    'manager_updated_successfully' => 'Manager updated successfully!',
    'imported_excel_for_employees' => 'Imported excel file for employees for company ',
    'employee_imported_successfully' => 'Employee successfully imported!',
    'exported_excel_for_employees' => 'Exported excel file for employees for company ',
    'exported_excel_for_all_employees' => 'Exported excel file for all employees',
    'employee_import_success_with_note' => 'Employee successfully imported, if nothing happened make sure, the email, department, services of employees are rightly mapped.!',
    'manager_created_successfully' => 'Manager created successfully!',
    'manager_updated_successfully' => 'Manager updated successfully!',

    // Additional Success Messages
    'employee_successfully_moved_to_trash' => 'Employee successfully moved to trash!',
    'employee_successfully_restored' => 'Employee successfully restored!',
    'employee_permanently_deleted' => 'Employee permanently deleted!',

    // Error Messages
    'cannot_permanently_delete_employee' => 'Cannot permanently delete employee. It has related records.',
    'cannot_permanently_delete_employees' => 'Cannot permanently delete the following employees as they have related records: ',
    'no_permission_assign_roles' => 'You do not have permission to assign the following roles: ',
    'maximum_2_roles_allowed' => 'A user can have a maximum of 2 roles.',

    // Bulk Operations
    'selected_employees_moved_to_trash' => 'Selected employees moved to trash!',
    'selected_employees_restored' => 'Selected employees restored!',
    'selected_employees_permanently_deleted' => 'Selected employees permanently deleted!',

    // Employee fields
    'first_name' => 'First Name',
    'last_name' => 'Last Name',
    'phone_number' => 'Phone Number',
    'email_address' => 'E-Mail Address',
    'matricule' => 'Matricule',
    'department' => 'Department',
    'company' => 'Company',
    'role' => 'Role',
    'services' => 'Services',
    'active' => 'Active',
    'inactive' => 'Inactive',
    'banned' => 'Banned',
    'is_active' => 'Is Active?',

    // Import/Export
    'company_required_for_import' => 'Company is required for employee import',
    'select_company_for_import' => 'Select the company where employees will be imported',
    'company_context_required' => 'Company context is required for employee import',
    'role_must_be_valid' => 'Role must be one of: employee, supervisor, manager',
    'import_completed' => 'Import completed successfully',

    // SMS/Email notifications
    'receive_sms_notifications' => 'Receive SMS notifications for payslips',
    'sms_notifications' => 'SMS notifications',
    'sms_notifications_description' => 'Enable or disable SMS notifications when payslips are sent to this employee',
    'sms_notifications_disabled' => 'SMS notifications disabled for this employee',
    'sms_notifications_enabled' => 'SMS notifications enabled for this employee',
    'enable_sms_notifications' => 'Enable SMS notifications',
    'disable_sms_notifications' => 'Disable SMS notifications',
    'bulk_sms_company_label' => 'Company-wide SMS notifications',
    'bulk_sms_select_company' => 'Select company for bulk SMS action',
    'bulk_sms_company_help' => 'Enable or disable SMS notifications for all active employees in the selected company.',
    'bulk_sms_disable_action' => 'Disable SMS for company',
    'bulk_sms_enable_action' => 'Enable SMS for company',
    'bulk_sms_disable_confirm' => 'Disable SMS notifications for all active employees in this company?',
    'bulk_sms_enable_confirm' => 'Enable SMS notifications for all active employees in this company?',
    'bulk_sms_disable_confirm_company_name' => 'Disable SMS notifications for all active employees in :company?',
    'bulk_sms_enable_confirm_company_name' => 'Enable SMS notifications for all active employees in :company?',
    'bulk_sms_select_company_first' => 'Please select a company first.',
    'bulk_sms_company_not_found' => 'Selected company was not found.',
    'bulk_sms_company_role_not_allowed' => 'Only admins and managers can perform company-wide SMS actions.',
    'bulk_sms_disabled_for_company' => 'SMS notifications disabled for :count employee(s) in :company.',
    'bulk_sms_enabled_for_company' => 'SMS notifications enabled for :count employee(s) in :company.',
    'bulk_email_company_help' => 'Enable or disable email notifications for all active employees in the selected company.',
    'bulk_email_disable_action' => 'Disable Email for company',
    'bulk_email_enable_action' => 'Enable Email for company',
    'bulk_email_disabled_for_company' => 'Email notifications disabled for :count employee(s) in :company.',
    'bulk_email_enabled_for_company' => 'Email notifications enabled for :count employee(s) in :company.',
    'manage_notifications_action' => 'Manage notifications',
    'manage_notifications_title' => 'Manage company notifications',
    'manage_notifications_help' => 'Choose a company and enable or disable SMS and email notifications for all its active employees.',
    'receive_email_notifications' => 'Receive Email notifications for payslips',
    'email_notifications' => 'Email notifications',
    'email_notifications_description' => 'Enable or disable Email notifications when payslips are sent to this employee',
    'email_notifications_disabled' => 'Email notifications disabled for this employee',
    'email_notifications_enabled' => 'Email notifications enabled for this employee',
    'enable_email_notifications' => 'Enable email notifications',
    'disable_email_notifications' => 'Disable email notifications',
    'notifications' => 'Notifications',
    'alternative_email' => 'Alternative Email',
    'alternative_email_description' => 'Optional backup email address',
    'alternative_email_note' => 'Optional: Backup email address to use if primary email fails',

    // Errors
    'user_matricule_empty' => 'User Matricule is empty',
    'not_associated_with_company' => 'You are not associated with any company. Please contact your administrator.',
    'not_associated_with_department' => 'You are not associated with any department. Please contact your administrator.',
    'payslip_file_not_found' => 'Payslip file not found. Please contact your administrator.',
    'unable_to_download_payslip' => 'Unable to download payslip. Please contact your administrator.',
    'all_employees' => 'All Employees',
    'all_employees_under_supervision' => 'All employees under my supervision',
    'new_manager' => 'New Manager',
    'service' => 'Service',
    'net_salary' => 'Net Salary',
    'leave_reason' => 'Leave Reason',
    'repayment_from_month' => 'Which month will start repayment?',
    'repayment_to_month' => 'Which month will end repayment?',

    // Beneficiary information
    'beneficiary_name' => 'Beneficiary Name',
    'beneficiary_mobile_money_number' => 'Beneficiary Mobile Money Number',
    'beneficiary_id_card_number' => 'Beneficiary ID Card Number',

    // Form fields
    'comments' => 'Comments',
    'approval_status' => 'Approval Status',
    'salary_grade' => 'Salary Grade',
    'select_manager' => 'Select Manager',

    // Employee Dashboard
    'hi' => 'Hi',
    'happy_birthday' => 'Happy Birthday 🎉, enjoy your day!',
    'no_company' => 'No Company',
    'no_department' => 'No Department',
    'checkins' => 'CheckIns',
    'advance_salary_short' => 'A. Salary',
    'absence' => 'Absence',
    'leave' => 'Leave',
    'checkins_checkout_recorded' => 'Checkins and Checkout recorded',
    'view_past_checkins' => 'You can view all your past checkins/chechout by clicking there!',
    'view_history' => 'View history',
    'overtime_recorded' => 'Overtime recorded',
    'view_all_overtime_records' => 'View all your overtime records!',
    'advance_salary' => 'Advance salary',
    'view_all_requests' => 'View all your request!',
    'absences' => 'Absences',
    'view_all_absences' => 'View all your absence!',
    'available_payslips' => 'Available payslips',
    'view_all_payslips_records' => 'View all your payslips records!',
    'remaining_leave_days' => 'Remaining Leave days',
    'latest_audit_logs' => 'Latest Audit logs',
    'view_all' => 'View all',
    'action_type' => 'Action Type',
    'action_performed' => 'Action Performed',
    'date' => 'Date',

    // Profile specific
    'profile_settings' => 'Profile Settings',
    'update_personal_details' => 'Update your personal details!',
    'personal_details' => 'Personal Details',
    'phone_number' => 'Phone Number',
    'personal_phone_number' => 'Personal Phone Number',
    'mobile_money_number' => 'Mobile Money Number',
    'pdf_password' => 'PDF Password',
    'work_time' => 'Work Time',
    'select_preferred_language' => 'Select your Preferred language for Notifications',
    'select_language' => 'Select Language',
    'english' => 'English',
    'french' => 'French',
    'add_signature' => 'Add Signature',
    'upload_signature' => 'Upload transparent/white background signature',
    'use_this_tool' => 'use this tool',
    'password_reset' => 'Password Reset',
    'current_password' => 'Current Password',
    'new_password' => 'New Password',
    'confirm_password' => 'Confirm password',
    'reset' => 'Reset',

    // Absences specific
    'absences_history' => 'Absences History',
    'view_all_absences_recorded' => 'View all your absences recorded so far',
    'request_absence' => 'Request Absence',
    'absence_date' => 'Absence Date',
    'view_attachment' => 'view attachment',
    'recorded_absence_message' => 'Recorded absence to see them here!',
    'absence_request_submitted' => 'Absence request successfully submitted - nice 😍!',
    'absence_request_updated' => 'Absence request updated successfully - nice 😍!!',
    'absence_deleted' => 'Absence successfully deleted!',
    'absence_restored' => 'Absence successfully restored!',
    'absence_permanently_deleted' => 'Absence permanently deleted!',
    'selected_absences_restored' => 'Selected absences restored!',
    'selected_absences_permanently_deleted' => 'Selected absences permanently deleted!',
    'absence_start_date' => 'Start Date',
    'absence_end_date' => 'End Date',
    'selected_absence_days' => 'Selected absence days: ',
    'absences_requests_submitted' => 'Absence requests submitted successfully for :count days - nice 😍!!',
    'cannot_request_past_absence' => 'Cannot request absence for past dates!',
    'upload_new_to_replace' => 'Upload a new file to replace the existing one (optional)',

    // Advance Salary specific
    'advance_salary' => 'Advance Salary',
    'view_all_advance_salary_requests' => 'View all your request for advance salary',
    'request_advance_salary' => 'Request Advance Salary',
    'beneficiary' => 'Beneficiary',
    'repayment_start' => 'Repayment Start',
    'repayment_end' => 'Repayment End',
    'currency_xaf' => 'XAF',
    'request_advance_salary_message' => 'Request Advance salary to see them here!',
    'advance_salary_recorded' => 'Advance salary recorded successfully!',
    'advance_salary_updated' => 'Advance Salary successfully updated!',
    'advance_salaries_bulk_updated' => 'Advance salaries successfully updated!',
    'advance_salary_single_updated' => 'Advance salary successfully updated!',
    'advance_salary_deleted' => 'Advance salary successfully deleted!',
    'advance_salary_moved_to_trash' => 'Advance salary successfully moved to trash!',
    'advance_salary_restored' => 'Advance salary successfully restored!',
    'advance_salary_permanently_deleted' => 'Advance salary permanently deleted!',
    'selected_advance_salaries_moved_to_trash' => 'Selected advance salary records moved to trash!',
    'selected_advance_salaries_restored' => 'Selected advance salaries restored!',
    'selected_advance_salaries_permanently_deleted' => 'Selected advance salaries permanently deleted!',
    'select_all_advance_salaries' => 'Select all advance salaries',
    'advance_salary_record' => 'advance salary record',
    'advance_salaries' => 'advance salaries',
    'repayment_period' => 'Repayment Period',

    // Portal Advance Salary specific
    'advance_salaries' => 'Advance salaries',
    'advance_salaries_management' => 'Advance salaries Management',
    'manage_advance_salaries_requests' => 'Manage Employees Advance salaries requests!',
    'total_advance_salaries' => 'Advance Salaries',
    'advance_salary_record' => 'advance salary record',
    'update_or_approve_advance_salary' => 'Update or Approve Advance Salary',
    'update_or_approve_employee_advance_salary_request' => 'Update or approve employee advance salary request',
    'bulk_advance_salary' => 'Bulk Advance Salary',
    'employee_advance_salary_bulk' => 'Employee advance salary bulk',

    // Advance Salary placeholders
    'amount_placeholder' => '25,000',
    'reason_placeholder' => 'To manage an urgent family matter',
    'beneficiary_name_placeholder' => 'Janette Jaqueline',
    'mobile_money_placeholder' => '6XXXXXXXX',
    'id_card_placeholder' => '12xxxxxxxx',

    // Loan repayment
    'loan_repayment' => 'Loan Repayment',
    'amount_repaid' => 'Amount repaid',
    'remaining_to_repay' => 'Remaining to repay',
    'fully_repaid' => 'Fully repaid',
    'record_repayment' => 'Record repayment',
    'repayment_amount_placeholder' => '25,000',
    'repayment_amount_required' => 'Please enter a valid repayment amount.',
    'repayment_recorded_successfully' => 'Repayment recorded successfully!',
    'repayment_only_for_loans' => 'Repayment can only be recorded for loans.',
    'loan_already_fully_repaid' => 'This loan is already fully repaid.',

    // Manager placeholders
    'first_name_placeholder' => 'John',
    'last_name_placeholder' => 'Doe',
    'matricule_placeholder' => '1134578',
    'email_placeholder' => 'example@company.com',
    'phone_placeholder' => '2376xxxxxxxxx',

    // Manager modal titles
    'update_manager_details' => 'Update Manager\'s details',
    'edit_and_update_manager_details' => 'Edit and update Manager details',

    // Audit Logs specific
    'view_all_activities_audit_logs' => 'View all activities audit Logs',
    'recorded' => 'recorded!',
    'record_audit_logs_message' => 'Record audit logs to see them here!',

    // Checklog specific
    'checkin_recorded' => 'Checkin Recorded',
    'view_all_checkins_recorded' => 'View all your checkin recorded so far',
    'single_checkin' => 'Single CheckIn',
    'bulk_checkin' => 'Bulk CheckIn',
    'checkin_time' => 'Checkin Time',
    'checkout_time' => 'Checkout Time',
    'sup_approval_status' => 'Sup Approval status',
    'mgr_approval_status' => 'Mgr Approval status',
    'expected_checkout_time' => 'Expected Checkout Time',
    'record_checkin_message' => 'Record a checkin to see it here!',
    'checkin_recorded_success' => 'Checkin recorded successfully - have a nice day 😍!',
    'checkins_recorded_success' => 'Checkins recorded successfully - have a nice day 😍!',
    'checkin_updated_success' => 'Checkin successfully updated!',
    'checkin_deleted_success' => 'Checkin record successfully deleted!',
    'checkin_restored' => 'Checkin successfully restored!',
    'checkin_permanently_deleted' => 'Checkin permanently deleted!',
    'selected_checkins_deleted' => 'Selected checkins deleted!',
    'selected_checkins_restored' => 'Selected checkins restored!',
    'selected_checkins_permanently_deleted' => 'Selected checkins permanently deleted!',
    'checkin_record' => 'checkin record',
    'checkins' => 'checkins',

    // Portal Checklog success messages
    'checkin_successfully_updated' => 'Checkin successfully updated!',
    'checkin_successfully_moved_to_trash' => 'Checkin successfully moved to trash!',
    'checkin_successfully_restored' => 'Checkin successfully restored!',
    'checkin_permanently_deleted' => 'Checkin permanently deleted!',
    'selected_checkin_records_moved_to_trash' => 'Selected checkin records moved to trash!',
    'selected_checkin_records_restored' => 'Selected checkin records restored!',
    'selected_checkin_records_permanently_deleted' => 'Selected checkin records permanently deleted!',

    // Portal Checklog specific
    'manage_employees_checkins' => 'Manage Employees checkins to you!',
    'total_checkins' => 'Checkins',
    'recorded_by_employees' => 'recorded by employees',
    'you_approved' => 'you approved!',
    'you_rejected' => 'you rejected!',
    'check_period' => 'Check Period',
    'checkout' => 'CheckOut',
    'time_worked' => 'Time Worked',
    'update_or_approve_checkin' => 'Update or Approve Checkin',
    'update_or_approve_employee_checkin_record' => 'Update or approve employee checkin record',
    'checkin_comment' => 'Checkin Comment',
    'employee_checkin_bulk' => 'Employee checkin bulk',

    // Leaves specific
    'leaves_history' => 'Leaves History',
    'view_all_leaves_recorded' => 'View all your leaves recorded so far',
    'request_leave' => 'Request Leave',
    'remaining_days' => 'remaining days',
    'monthly_allocation' => 'monthly allocation',
    'days' => 'days',
    'remaining_leave_days_allocation' => 'Remaining Leave days and monthly leave allocation',
    'leave_balance' => 'Leave balance',
    'available' => 'Available',
    'used' => 'Used',
    'remaining' => 'Remaining',
    'leave_days_taken' => 'leave days taken so far!',
    'all_leave_days_taken' => 'All leave days taken so far!',
    'start_date' => 'Start Date',
    'end_date' => 'End date',
    'number_of_days' => 'Number of Days',
    'leave_type' => 'Leave Type',
    'period' => 'Period',
    'sup_approval_status' => 'Sup Approval status',
    'mgr_approval_status' => 'Mgr Approval status',
    'record_leave_message' => 'Record leave to see them here!',
    'selected_leave_days' => 'Selected Leave days are ',
    'start_date_before_end' => 'Start date must be less than End date',
    'leave_request_submitted' => 'Leave request successfully submitted - nice 😍!',
    'leave_request_updated' => 'Leave request updated successfully - nice 😍!!',
    'leave_deleted' => 'Leave successfully deleted!',
    'leave_restored' => 'Leave successfully restored!',
    'leave_permanently_deleted' => 'Leave permanently deleted!',
    'selected_leaves_deleted' => 'Selected leaves deleted!',
    'selected_leaves_restored' => 'Selected leaves restored!',
    'selected_leaves_permanently_deleted' => 'Selected leaves permanently deleted!',
    'leave_record' => 'leave record',
    'leaves' => 'leaves',

    // Overtime specific
    'view_all_overtime_recorded' => 'View all your overtime recorded',
    'create_overtime' => 'Create Overtime',
    'record_overtime_message' => 'Record overtime worked to see them here!',
    'overtime_created' => 'Overtime successfully created!',
    'overtime_updated' => 'Overtime successfully updated!',
    'overtime_deleted' => 'Overtime record successfully deleted!',
    'overtime_restored' => 'Overtime successfully restored!',
    'overtime_permanently_deleted' => 'Overtime permanently deleted!',
    'selected_overtimes_deleted' => 'Selected overtimes deleted!',
    'selected_overtimes_restored' => 'Selected overtimes restored!',
    'selected_overtimes_permanently_deleted' => 'Selected overtimes permanently deleted!',
    'overtime_record' => 'overtime record',
    'overtimes' => 'overtimes',

    // Payslip specific
    'view_all_payslips' => 'View all your payslips',
    'year' => 'Year',
    'month' => 'Month',
    'sent_date' => 'Sent Date',
    'download' => 'Download',
    'record_payslip_message' => 'Record payslip to see them here!',

    // Modal translations
    'create_an_absence' => 'Create an Absence',
    'record_and_submit_an_absence' => 'Record and submit an absence',
    'absence_date' => 'Absence Date',
    'supporting_file' => 'Supporting File',
    'update_an_absence' => 'Update an Absence',
    'edit_and_update_absence' => 'Edit and update absence',

    'request_for_leave' => 'Request for Leave',
    'submit_a_leave_request' => 'Submit a Leave Request',
    'select' => '--Select--',
    'leave_start_date' => 'Leave Start Date',
    'leave_end_date' => 'Leave End Date',

    'request_advance_salary' => 'Request Advance Salary',
    'create_and_submit_advance_salary_request' => 'Create and submit an advance salary request',
    'repayment_from_month' => 'Repayment From Month',
    'repayment_to_month' => 'Repayment To Month',
    'advance_salary_request_only_1_to_12' => 'Advance salary requests can only be made between the 1st and 12th of each month.',
    'advance_salary_repayment_info' => 'The advance salary will be repaid on this same month\'s payroll.',
    'advance_type' => 'Type',
    'advance_payment' => 'Advance Payment',
    'loan' => 'Loan',
    'advance_for_month' => 'Advance For Month',
    'one_advance_per_month' => 'Only one advance payment request per month is allowed.',
    'supporting_documentation' => 'Supporting Documentation',
    'loan_attachment_required' => 'Required for loan requests (png, jpg, pdf, doc, docx, max 5MB).',
    'loan_previous_not_repaid' => 'You cannot request a new loan until the previous one is fully repaid.',
    'advance_already_requested_for_month' => 'You have already requested an advance for this month.',
    'supervisor_approval' => 'Supervisor Approval',
    'manager_approval' => 'Manager Approval',
    'optional_attachment' => 'Optional supporting document (png, jpg, pdf, doc, docx, max 5MB).',
    'quick_add_overtime' => 'Quick add: enter hours only',
    'overtime_hours' => 'Hours',
    'quick_hours_help' => 'Hours worked after your regular work end time.',
    'repayment_end_before_start' => 'The repayment end month cannot be before the start month.',
    'repayment_cannot_be_in_past' => 'The repayment start month cannot be in the past.',
    'submit_request' => 'Submit Request',

    'checkin' => 'CheckIn',
    'record_checkin' => 'Record CheckIn',
    'record_checkin_to_track_work' => 'Record checkin to track your work',
    'checkin_date' => 'CheckIn Date',
    'start_time' => 'Start Time',
    'end_time' => 'End Time',
    'checkin_comments' => 'CheckIn Comments',

    'bulk_checkin' => 'Bulk CheckIn',
    'record_bulk_checkin' => 'Record Bulk CheckIn',
    'bulk_checkins' => 'Bulk Checkins',
    'record_bulk_checkins_for_period' => 'Record bulk checkins for a given period.',
    'start_day' => 'Start Day',
    'end_day' => 'End Day',
    'start_date' => 'Start Date',
    'end_date' => 'End Date',
    'checkin_time' => 'Checkin time',
    'checkout_time' => 'Checkout time',

    'edit_checklog' => 'Edit Checklog',
    'edit_checklog_details' => 'Edit Checklog Details',
    'update_checkin' => 'Update Checkin',
    'update_checkin_record' => 'Update checkin record',

    'create_overtime' => 'Create Overtime',
    'record_overtime_worked' => 'Record overtime worked',
    'record_and_submit_overtime' => 'Record and submit any overtime you did',
    'submit_overtime' => 'Submit Overtime',
    'submit_overtime_request' => 'Submit Overtime Request',
    'overtime_date' => 'Overtime Date',
    'overtime_reason' => 'Overtime Reason',

    'edit_overtime' => 'Edit Overtime',
    'update_overtime' => 'Update Overtime',
    'edit_and_update_overtime_request' => 'Edit and update Overtime request',
    'edit_overtime_details' => 'Edit Overtime Details',

    'update_leave_request' => 'Update Leave Request',
    'edit_and_update_leave_request' => 'Edit and update Leave Request',

    'update_advance_salary_request' => 'Update Advance Salary Request',
    'edit_and_submit_advance_salary_request' => 'Edit and submit an advance salary request',
    'update_request' => 'Update Request',

    // Livewire component messages
    'not_associated_with_company' => 'You are not associated with any company. Please contact your administrator.',
    'payslip_file_not_found' => 'Payslip file not found. Please contact your administrator.',
    'unable_to_download_payslip' => 'Unable to download payslip. Please contact your administrator.',

    // Role management specific
    'no_permission_assign_roles' => 'You do not have permission to assign the following roles: ',
    'maximum_2_roles_allowed' => 'A user can have a maximum of 2 roles.',

    // Portal employee management
    'employees' => 'Employees',
    'email' => 'Email',
    'department_employees' => 'Department Employees',
    'manage_employees_in_assigned_departments' => 'Manage employees in your assigned departments',
    'manage_employees_for' => 'Manage Employees for ',
    'you_manage' => 'you manage',
    'recorded_by_manager' => 'recorded by manager',
    'who_are_active' => 'who are active!',
    'create_new_employee' => 'Create a new Employee',
    'create_new_employee_in_department' => 'Create a new employee in your department',
    'create_new_employee_for' => 'Create a new employee for',
    'department_company' => 'Department Company',
    'select_department' => 'Select department',
    'select_service' => 'Select service',
    'update_employee' => 'Update Employee',
    'update_employee_details' => 'Update employee details',
    'unknown_company' => 'Unknown Company',
    'you_manage' => 'you manage',
    'manager' => 'Manager',
    'approved' => 'Approved',
    'leave' => 'leave',
    'leaves' => 'leaves',
    'pending_approval' => 'Pending Approval',
    'all_employees' => 'All Employees',
    'all_employees_under_my_supervision' => 'All employees under my supervision',
    'new_manager' => 'New Manager',
    'total_employees' => 'Employees',
    'who_are_banned' => 'who are banned!',
    'details' => 'Details',
    'created' => 'Created',
    'pdf_password' => 'PDF Password',
    'professional_phone' => 'Professional Phone',
    'personal_phone' => 'Personal Phone',
    'manage_roles' => 'Manage Roles',
    'restore_employee' => 'Restore Employee',
    'permanently_delete' => 'Permanently Delete',
    'na' => 'NA',
    'department_employees' => 'Department Employees',
    'manage_employees_in_assigned_departments' => 'Manage employees in your assigned departments',

    // Bulk Operations
    'select_all_employees' => 'Select All Employees',
    'select_all_deleted_employees' => 'Select All Deleted Employees',
    'restore_selected_employees' => 'Restore Selected Employees',
    'permanently_delete_selected_employees' => 'Permanently Delete Selected Employees',

    // Employee Form Labels
    'total_leave_days' => 'Leave Days',
    'monthly_allocations' => 'Monthly Allocations',
    'contract_end_date' => 'Contract End Date',
    'create_employee_password' => 'Create Employee\'s Password',
    'reset_employee_password' => 'Reset Employee\'s Password',
    'password_auto_generated_info' => 'A password will be automatically generated and sent to the user by email. They can change it after logging in.',
    'add_employee' => 'Add Employee',
    'add_to' => 'Add to',
    'banned' => 'Banned',
    'update_employee' => 'Update Employee',
    'create_a_new_employee' => 'Create a new Employee',
    'import_employee_for_company' => 'Import :name for your company.',
    'fill_template_auto_add_details' => 'Fill template with your :name data, note that department and company details will be auto added',

    // User Roles Management
    'user_roles' => 'User Roles',
    'assigned_roles' => 'Assigned Roles',
    'no_roles_assigned' => 'No roles assigned',
    'manage_roles' => 'Manage Roles',
    'select_roles_max_2' => 'Select Roles (Max 2)',
    'update_roles' => 'Update Roles',
    'note' => 'Note:',
    'maximum_2_roles_per_user_allowed' => 'Maximum 2 roles per user allowed',
    'employee_role_automatically_included' => 'Employee role is automatically included',
    'use_ctrl_cmd_click_multiple_selection' => 'Use Ctrl/Cmd + Click for multiple selection',
    'are_you_sure_remove_role' => 'Are you sure you want to remove this role?',
    'cannot_remove' => 'Cannot remove',
    'employee_role_must_always_be_included' => 'Employee role must always be included.',
    'user_maximum_2_roles_including_employee' => 'A user can have a maximum of 2 roles (including the employee role).',

    // Manager Creation
    'create_a_new_manager' => 'Create a new Manager',

    // Error Messages
    'department_access_required_for_supervisors' => 'Department access required for supervisors.',
    'no_access_to_department' => 'You do not have access to this department.',
    'no_permission_access_company' => 'You do not have permission to access this company.',

    // Form Helpers
    'leave_empty_to_keep_current_password' => 'Leave empty to keep current password',

    // Additional keys
    'employee_info' => 'Employee Info',

    // Import validation keys
    'first_name_required' => 'First name is required',
    'last_name_required' => 'Last name is required',
    'at_least_one_name_required' => 'At least one of first name or last name must be provided',
    'email_required' => 'Email is required',
    'email_already_exists' => 'Email already exists',
    'professional_phone_required' => 'Professional phone number is required',
    'matricule_required' => 'Matricule is required',
    'matricule_already_exists' => 'Matricule already exists',
    'matricule_already_used_by' => 'This matricule is already assigned to :email.',
    'email_local_part_already_used' => 'The part before @ in this email is already used by :email. Use the existing account or update that employee instead.',
    'position_required' => 'Position is required',
    'net_salary_required_numeric' => 'Net salary is required and must be numeric',
    'salary_grade_required' => 'Salary grade is required',
    'department_required' => 'Department is required',
    'service_required' => 'Service is required',
    'role_required' => 'Role is required',
    'role_invalid' => 'Role is invalid. Must be employee, supervisor, or manager',
    'work_start_time_invalid_format' => 'Work start time must be in HH:MM format',
    'work_end_time_invalid_format' => 'Work end time must be in HH:MM format',
    'work_start_time_must_be_before_end' => 'Work start time must be before work end time',
    'personal_phone_invalid' => 'Personal phone number is invalid',
    'professional_phone_invalid' => 'Professional phone number is invalid',
    'alternative_email_invalid' => 'Alternative email is invalid',

    // Supervisor/manager import translations
    'email_is_empty' => 'Supervisor email is empty',
    'not_found' => 'Supervisor with email :email not found',
    'does_not_have_role' => 'User with email :email does not have supervisor or manager role',

    // Background import completion messages
    'background_import_completed' => 'Background import completed successfully. :count records imported.',

    // Report translations
    'employee_name' => 'Employee Name',
    'qualification' => 'Qualification',
    't_hours_worked' => 'Total Hours Worked',
    'signature' => 'Signature',

    // Additional translation keys
    'attachment_link' => 'View attachment',
    'bulk_leave' => 'Bulk leave',
    'date_of_birth' => 'Date of birth',
    'email_invalid' => 'The email address is invalid',
    'employee_leave_bulk' => 'Employee bulk leave',
    'invalid_role' => 'Invalid role',
    'invalid_work_end_time' => 'Invalid work end time',
    'invalid_work_start_time' => 'Invalid work start time',
    'monthly_leave_allocation' => 'Monthly leave allocation',
    'no_absence_selected' => 'No absence selected',
    'no_advance_salary_selected' => 'No advance salary selected',
    'no_checklog_selected' => 'No check log selected',
    'no_employee_selected' => 'No employee selected',
    'no_leave_selected' => 'No leave selected',
    'no_overtime_selected' => 'No overtime selected',
    'overtime' => 'Overtime',
    'position' => 'Position',
    'work_end_time' => 'Work end time',
    'work_start_time' => 'Work start time',
];
