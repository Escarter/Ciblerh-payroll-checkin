    ...
    <div class=''>
        <div class='' style="padding-bottom: 10px;">
            ...
            <p style="margin-left: 63%; width: 38%; font-size:smaller;">
                <span style="font-size: x-larger; font-weight:bolder; ">{{__('reports.attachment')}} <small style="font-size: small; color:red;">{{\Str::random(10)}}</small></span>
                <br />{{__('reports.period_from')}} {{ now()->isoweek((int) $week)->startOfWeek()->ISOFormat('L')}} {{__('common.to')}} {{ now()->isoweek((int) $week)->endOfWeek()->ISOFormat('L') }}
                <br />{{__('reports.client')}} : {{!is_null($company) ? $company->name : ''}}
                <br>{{__('departments.department')}} : {{!is_null($department) ? $department->name : ''}}
            </p>
            <p class="text-underline" style="margin-left: 45%; text-align: center; font-weight:bolder; ">{{__('reports.overtimes_recorded')}}</p>
        </div>
