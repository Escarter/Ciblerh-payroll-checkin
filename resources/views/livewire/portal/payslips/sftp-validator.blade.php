                            ...
                            <td class="text-end pe-3">
                                <div class="d-flex gap-2 justify-content-end align-items-center">
                                    ...
                                    @elseif ($proposal->status === 'validated')
                                        {{-- Process --}}
                                        <a href="#"
                                            wire:click.prevent="processSingle('{{ $proposal->id }}')"
                                            wire:loading.class="opacity-50 pe-none"
                                            wire:target="processSingle('{{ $proposal->id }}')"
                                            class="text-success me-1" title="{{ __('payslips.process_selected') }}">
                                            <svg wire:loading.remove wire:target="processSingle('{{ $proposal->id }}')" class="icon icon-xs" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                                            </svg>
                                            <svg wire:loading wire:target="processSingle('{{ $proposal->id }}')" class="icon icon-xs" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="animation:spin .8s linear infinite">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/>
                                            </svg>
                                        </a>
                                    @endif
    ...
    <div class="modal fade" id="viewProposalModal" tabindex="-1" wire:ignore.self>
        <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    @if ($viewingProposal?->status === 'pending')
                        <button type="button"
                            class="btn btn-warning"
                            wire:click="rematch('{{ $viewingProposal->id }}')"
                            wire:loading.attr="disabled"
                            wire:target="rematch('{{ $viewingProposal->id }}')">
                            <svg wire:loading.remove wire:target="rematch('{{ $viewingProposal->id }}')" class="icon icon-xs me-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                            </svg>
                            <svg wire:loading wire:target="rematch('{{ $viewingProposal->id }}')" class="icon icon-xs me-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="animation:spin .8s linear infinite">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/>
                            </svg>
                            {{ __('payslips.rematch') }}
                        </button>
                    @elseif ($viewingProposal?->status === 'failed')
                        <button type="button"
                            class="btn btn-danger"
                            wire:click="retryFailedProposal('{{ $viewingProposal->id }}')"
                            wire:loading.attr="disabled"
                            wire:target="retryFailedProposal('{{ $viewingProposal->id }}')">
                            <svg wire:loading.remove wire:target="retryFailedProposal('{{ $viewingProposal->id }}')" class="icon icon-xs me-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                            </svg>
                            <svg wire:loading wire:target="retryFailedProposal('{{ $viewingProposal->id }}')" class="icon icon-xs me-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="animation:spin .8s linear infinite">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/>
                            </svg>
                            {{ __('payslips.retry_failed_proposal') }}
                        </button>
                    @endif
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ __('common.close') }}</button>
                </div>
