     ...
     <div class="modal-dialog modal-dialog-centered modal-xl" style="max-width: 950px;">
         <div class="modal-content">
             <div class="modal-header">
                 <h4 class="modal-title fw-bold" id="importEmployeesModalLabel">
                     <span x-show="currentStep === 'upload'">{{__('common.import_name',['name'=>__('employees.employee')])}}</span>
                 ...
                 <div x-show="currentStep === 'upload'" x-transition x-transition.duration.300ms>
                     <div class="px-3 pt-1 pb-3">
                         <x-form-items.form class="form-modal">
                             <div class='mb-4'>
                                 <ol>
                                     <li class="mb-2">{{__('common.download_sample_import_template',['name'=>__('employees.employee')])}}
                                         <a href="{{asset('templates/employee_import_template.csv')}}" class="btn btn-sm btn-outline-success ms-2" download>
                                         ...
                                         <a href="{{asset('templates/import_employees.xlsx')}}" class="btn btn-sm btn-outline-secondary ms-1" download>
                                             <svg class="icon icon-xs me-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                                             </svg>
                                     ...
                                     <li class="mb-2">
                                         <strong>{{__('common.fill_template_instructions')}}</strong>
                                         <ul class="mt-2 mb-2">
                                             <li>{{__('common.use_names_not_ids')}}</li>
                                             <li>{{__('common.notification_preferences')}}</li>
                                             <li>{{__('common.phone_format_instruction')}}</li>
                                             <li>{{__('common.time_format_instruction')}}</li>
                                             <li>{{__('common.date_format_instruction')}}</li>
                                             <li>{{__('common.role_format_instruction')}}</li>
                                         </ul>
                                     </li>
                                     <li class="mb-2">{{__('common.upload_and_validate')}}</li>
                                 </ol>
                             </div>

                             <div class="mb-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" wire:model.live="autoCreateEntities" id="autoCreateEntities">
                                    <label class="form-check-label" for="autoCreateEntities">
                                        <strong>{{__('common.auto_create_entities')}}</strong>
                                    </label>
                                </div>
                             ...
                             <div class="mb-3" x-show="!$wire.company">
                                 ...
                                 <select wire:model.live="selectedCompanyId" class="form-select @error('selectedCompanyId') is-invalid @enderror" id="selectedCompanyId" required>
                                     ...
                                     @foreach(\App\Models\Company::orderBy('name')->get() as $companyOption)
                                     <option value="{{$companyOption->id}}">{{$companyOption->name}}</option>
                                     @endforeach
                                     @else
                                     @foreach(auth()->user()->managerCompanies ?? [] as $companyOption)
                                     <option value="{{$companyOption->id}}">{{$companyOption->name}}</option>
                                     @endforeach
                                     @endif
                                 </select>
                                 <div class="form-text">{{__('employees.select_company_for_import')}}</div>
                                 ...
                                 <div class="card">
                                     <div class="card-body">
                                         ...
                                         <ul class="list-unstyled">
                                             <li>
                                                 <strong>{{ __('common.total_rows') }}:</strong> {{ $totalRows }}
                                                 @if($isRowCountEstimated)
                                                     <small class="text-muted">({{ __('common.estimated') }})</small>
                                                 @endif
                                             </li>
                                             <li><strong>{{ __('common.valid_rows') }}:</strong>
                                                 @if(!empty($previewData))
                                                     <span class="text-success">{{ count(array_filter($previewData, fn($row) => $row['validation']['valid'] ?? false)) }}</span>
                                                 @else
                                                     <span class="text-muted">{{ __('common.not_available') }}</span>
                                                 @endif
                                             </li>
                                             <li><strong>{{ __('common.rows_with_errors') }}:</strong>
                                                 @if(!empty($previewData))
                                                     <span class="text-danger">{{ count($previewErrors) }}</span>
                                                 @else
                                                     <span class="text-muted">{{ __('common.not_available') }}</span>
                                                 @endif
                                             </li>
